...
async function fetchRandomPerson() {
  ...
  async function fetchRandomQuote() {
    ...
    let person: Person;
    try {
      const fetchedQuote = await axiosClient.get(`/quotes/${id}`);
      const quote = fetchedQuote.data.quote;
      const fetchedResult = await axiosClient.get(
        `/persons/${fetchedQuote.data.person_id}`
      );
      person = fetchedResult.data;
      if (person) {
        const fetchedGender = await axiosClient.get(
